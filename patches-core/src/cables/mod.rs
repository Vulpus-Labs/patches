...
// directly each tick; modules access them via `CablePool` using the
// `cable_idx` constants below. All slots carry `CableValue::Mono` unless
// noted.
...
/// demand stays below this cap sees identical backplane and sink
/// indices regardless of fusion topology growth.
pub const SCRATCH_CAPACITY: usize = 2048;

/// Total capacity of the cycle region (ADR 0072 phase 3, tickets 0850
/// + 0858, phase 5 invert ticket 0860).
///
/// Cycle slots live at `cable_idx` in `[SCRATCH_CAPACITY,
/// SCRATCH_CAPACITY + CYCLE_CAPACITY)` and are `[CableValue; 2]`
/// ping-pong pairs backing delayed feedback producers. Patches whose
/// cycle count exceeds `CYCLE_CAPACITY` raise `BufferPoolExhausted` at
/// plan-build time.
