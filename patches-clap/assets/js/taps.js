...
let slotWidgets = Object.create(null); // slot → { meter, scope, spectrum }
let lastTapsKey = null;
...
function rebuildTaps(taps) {
  ...
  const foldedRows = foldStereoPairs(sorted);

  for (const entry of foldedRows) {
    if (entry.stereo) {
...
api._renderTaps = (frame) => {
  ...
  for (const sl of frame.slots) {
    ...
    // animates the decay.
    if (sl.t === true) {
      triggerFireTime[sl.s] = performance.now();
    }
  }
};
