...
use crate::wav_write::{commit_wav_sizes, finalize_wav, write_wav_header};
...
fn run_writer(
    ...
    if let Err(e) = write_wav_header(&mut writer, 2, 16, sample_rate) {
    ...
    loop {
        ...
        if frames_written.saturating_sub(frames_at_last_commit) >= sample_rate
            && commit_wav_sizes(&mut writer, 2, 16, frames_written).is_ok()
        {
    ...
    if let Err(e) = finalize_wav(&mut writer, 2, 16, frames_written) {
