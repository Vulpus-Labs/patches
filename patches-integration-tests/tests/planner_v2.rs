...
use patches_engine::{ReadyState, StaleState, ModulePool};
use patches_modules::{AudioOut, Console, Oscillator, Sum};
...
fn shape_change_forces_re_instantiation() {
    ...
    // First build: Console with 2 channels.
    let mut graph_a = patches_core::ModuleGraph::new();
    graph_a.add_module(
        "mix",
        patches_core::describe_for::<Console>(&ModuleShape { channels: 2 }),
    ...
    graph_b.add_module(
        "mix",
        patches_core::describe_for::<Console>(&ModuleShape { channels: 4 }),
        &patches_core::parameter_map::ParameterMap::new(),
    ).unwrap();
    graph_b.add_module(
        "out",
