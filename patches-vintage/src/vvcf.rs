//! `VVcf` — vintage 4-pole ZDF ladder low-pass with Juno-60 / Juno-106 voicings.
//!
...
params_enum! {
    pub enum VVcfVariant {
        Juno60 => "juno60",
        Juno106 => "juno106",
    }
}

impl From<VVcfVariant> for LadderVariant {
    fn from(v: VVcfVariant) -> Self {
        match v {
            VVcfVariant::Juno60 => LadderVariant::Juno60,
            VVcfVariant::Juno106 => LadderVariant::Juno106,
        }
    }
}
...
impl Module for VVcf {
    fn describe(shape: &ModuleShape) -> ModuleDescriptor {
        ModuleDescriptor::new("VVcf", shape.clone())
            .mono_in("in")
            .mono_in("cutoff_cv")
            .mono_out("out")
...
mod tests {
    ...
    #[test]
    fn descriptor_shape() {
        let h = ModuleHarness::build::<VVcf>(&[]);
        let d = h.descriptor();
        assert_eq!(d.module_name, "VVcf");
    ...
    fn self_oscillates_at_max_resonance() {
        let mut h = ModuleHarness::build::<VVcf>(params![
