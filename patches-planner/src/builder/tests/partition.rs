...
fn partition_scale_one_goes_to_unscaled() {
    let (unscaled, scaled) = partition_inputs(vec![(5, 1.0, false), (7, 1.0, false)]);
...
fn partition_non_one_scale_goes_to_scaled() {
    let (unscaled, scaled) = partition_inputs(vec![(3, 0.5, false)]);
    assert!(unscaled.is_empty());
...
fn partition_mixed_produces_correct_split() {
    let (unscaled, scaled) = partition_inputs(vec![(2, 1.0, false), (4, 0.25, false), (6, 1.0, false), (8, -1.0, false)]);
